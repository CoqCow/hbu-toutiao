   ...
   // private Jedis jedis = null;
    ...
    public void afterPropertiesSet() throws Exception {
       ...
       // pool = new JedisPool("localhost", 6379);
        pool = new JedisPool("47.95.10.158", 6379);
    }
    public static void main(String[] args) {
        JedisAdapter adapter=new JedisAdapter();
        adapter.addMial("11556@qq.com","12345");
       // System.out.println(adapter.getCodeByEmail("11556@qq.com"));
        //print(1,adapter.getCodeByEmail("11556@qq.com"));

/*        Jedis jedis = null;
        JedisPool pool=new JedisPool("47.95.10.158", 6379);
        try {
            jedis = pool.getResource();
            jedis.auth("niu");//认证权限
            print(1,jedis.get("myKey"));
        } catch (Exception e) {
...
/*        jedis.set("hello", "world");
    ...
    public void addMial(String key, String value){
        Jedis jedis = null;
        try {
            jedis = pool.getResource();
            jedis.auth("niu");//认证权限
            jedis.setex(key,120,value);//保存两分钟
        } catch (Exception e) {
            logger.error("发生异常" + e.getMessage());
            e.printStackTrace();
        } finally {
    ...
    public String getCodeByEmail(String key){
        Jedis jedis = null;
        try {
            jedis = pool.getResource();
            jedis.auth("niu");//认证权限
